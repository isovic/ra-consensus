...
def extract_same_contigs(single_contig_file, reads_file, out_alt_ctg_file, ref_file=''):
	...
	# path_alt_contig_sams = '%s/temp/tmp.altctgs.sam' % (os.path.dirname(out_alt_ctg_file));

	# path_raw_contig = '%s/tmp.rawcontig.fasta' % (os.path.dirname(out_alt_ctg_file));
	path_aligns_basename = '%s/tmp.allreads' % (os.path.dirname(out_alt_ctg_file));
	path_aligns = '%s.sam' % (path_aligns_basename);
	# path_aligns_sorted = '%s/tmp.allreads.sorted.sam' % (os.path.dirname(out_alt_ctg_file));
	path_aligns_sorted_basename = '%s.sorted' % (path_aligns_basename);
	path_aligns_sorted_sam = '%s.sam' % (path_aligns_sorted_basename);
	# path_alt_contig_sams = '%s/tmp.altctgs.sam' % (os.path.dirname(out_alt_ctg_file));
	...
	# execute_command('/home/isovic/work/eclipse-workspace/graphmap/bin/graphmap-not_release -a anchor -b 3 -r %s -d %s -o %s' % (single_contig_file, reads_file, path_aligns));
	...
	while (coverage < 100 and len(sams_to_process) > 0):
		...
		if (float(non_clipped_len) < 0.95*float(len(ctg_seqs[0]))):
			# print 'Tu sam!';
			# exit(1);
			continue;
		else:
...
def process_contigs(contigs_file, reads_file, out_file):
	...
	while True:
		...
		extract_same_contigs(single_contig_file, reads_file, out_alt_ctg_file);
